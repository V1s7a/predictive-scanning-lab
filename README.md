...
## Coverage–vs–Probes: Natural Trade-offs

![Coverage vs Probes (Weighted Policy)](results/coverage_vs_probes_weighted.png)

**Figure. Coverage vs probes with risk-weighted selection (4 hosts, seeds = {22,80,443,445}).** Curves are monotone: larger probe budgets yield higher coverage. **Mode 1** selects the top-B (host,port) pairs by utility (probability × risk weight). **Mode 2** enforces a small must-include policy (e.g., AD/Windows critical ports) and then trims to the same budget. The dashed line marks the **full scan (88 probes)**. The curves are generated **without** any “oracle” forcing of missed opens—this reflects the **natural trade-off** between stealth (fewer probes) and completeness (higher coverage).

**What this shows.**  
- Both curves rise as we increase the probe **budget** \(B\).  
- **Mode 2** dominates Mode 1 at the same probe count (policy constraints help).  
- Practical “knee”: around **50–60 probes** we reach **~90%+ coverage**, still far below the **88-probe** full sweep.  
- Plateau <100% is expected here because we didn’t force in missed opens; this is the **natural** policy behavior on a small lab.

The figure is produced from `results/budget_sweep_weighted.csv`
